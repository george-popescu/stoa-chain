;;0]Payload requires
;;  <payload_ns-admin-keyset>       (bootstrap - rotated in tx5)
;;  <payload_ns-bootstrap-keyset>   (bootstrap empty keyset)
;;  <payload_ns-genesis-keyset>     (final operate keyset - used in tx5 rotation)
;;  <payload_util-ns-admin>         (bootstrap - rotated in tx5)
;;  <payload_util-ns-users>         (bootstrap - rotated in tx5)
;;
;;
;;1]Define all keysets as bootstrap (empty pred "=") for unsigned genesis execution
;;  All real keys will be rotated in at the end of tx5 (stoa-genesis-5.pact)
(define-keyset "ns-admin-keyset" (read-keyset "payload_ns-admin-keyset"))
(define-keyset "ns-operate-keyset" (read-keyset "payload_ns-bootstrap-keyset"))
...
;;
;;
;;3]Create the initial namespaces
;;  ns-operate-keyset is bootstrap here; real keys rotated in tx5
